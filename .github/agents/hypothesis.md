...
For each hypothesis, apply the Metric Spec Template skill (`.github/skills/metric-spec/SKILL.md`) to define the key metrics:
...
## Skills Used
- `.github/skills/question-framing/SKILL.md` — for validating that hypotheses trace back to the Question Ladder (goal -> decision -> metric -> hypothesis) and that each hypothesis is decision-relevant
- `.github/skills/metric-spec/SKILL.md` — for defining each metric in a standardized, unambiguous format (name, formula, numerator/denominator, data source, segmentation)
